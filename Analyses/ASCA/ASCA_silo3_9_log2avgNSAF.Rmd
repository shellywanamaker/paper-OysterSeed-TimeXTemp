...
```{r}
global_PC12 <- read.table("../../Data/global_PC12_proteins.txt", stringsAsFactors = FALSE, header = TRUE)
#rename column 1
...
ColSideColors<-cbind(Temp=c("brown",rep(c("cyan3","magenta2"),6)), Day=c("white",rep(c("#D9D9D9","#BDBDBD","#969696","#737373","#525252","#252525"),2)))

#plot the heat map

heatmap3(as.matrix(t(global_cut_data)), Colv = NA, cexRow = 0.5,labCol = c(19,21,21,23,23,25,25,27,27,29,29,31,31), ColSideColors = ColSideColors, ColAxisColors = 1)



#what are the proteins that distinguish day 21 and day 27?
hm <- as.dist(1-cor(global_cut_data, use="pa"))
...
jpeg(file="../../Figures/AdditionalFile1/FigureS2e.jpg", width=7, height=5, units = "in", res = 300)
...
write.csv(clade.prot,"PCA_d21d27.csv", quote = FALSE, row.names = FALSE)
...
jpeg(file="../../Figures/MainText/Figure3c.jpg", width=25, height=5, units = "in", res = 300)
...
time x temperature interaction
```{r}
tempxtime_PC12 <- data.frame(ASCA$`12`$svd$t[,1])
tempxtime_PC12 <- cbind(data.frame(ASCA$`12`$level.combinations$row.patterns), tempxtime_PC12)
tempxtime_PC12 <- cbind(tempxtime_PC12, data.frame(ASCA$`12`$svd$t[,2]))
colnames(tempxtime_PC12) <- c("dpf", "temp","PC1","PC2")

ggplot(tempxtime_PC12, aes(x = PC1, y = PC2, color = as.factor(temp), label = as.factor(dpf))) + geom_text(fontface = "bold", size = 5) + stat_ellipse(type = "t", linetype = 1) + theme_bw() 
